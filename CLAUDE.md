...
- **Прогресс** — per-user, таблица `progress`, ключ `(userId, lessonKey, pt)`,
  ...
  **Важно:** интервал SM-2 двигается ТОЛЬКО на «событие повторения» — когда
  ...
  ограничен сверху `MAX_INTERVAL=120` дн. (≈«через 4 месяца»; для A0–A1 повтор
  хотя бы раз в ~4 месяца — «вечного выпуска» слова из ротации нет). Легаси-строки
  с раздутым (досхемным) `due` чинит
  одноразовая идемпотентная миграция `reclampSchedules` (`npx convex run
  progress:reclampSchedules`, на проде — c `--prod`) — обрезает `interval`/`due`
  под потолок. Классификация due/new/learned/ongoing — в `getSrsState`.
...
- **UI тренировки.** Хедер ([`Header.tsx`](src/components/Header.tsx)) — только
  логотип слева; справа по порядку тема, стрик, «выйти» (выйти — крайний правый).
  ...
  (до-ответную строку «следующий повтор …» показываем только для реально
  `due`-слов — на досрочной практике «Тренировать все слова» будущая дата это
  шум; плюс срез нижнего паддинга `.m-app`), чтобы кнопка «Дальше» помещалась

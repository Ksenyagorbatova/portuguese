...
// Аудио-экстра в фазе изучения (П.1): пока слово ещё осваивается (не выучено),
// но уже узнано хотя бы раз (mc≥1), с этой вероятностью вместо обычного показа
// даём аудио-карточку СВЕРХ программы — тренировка слуха. Она НЕ двигает
// выученность (серверный mode "audio" не растит mc/type), поэтому, как и relief,
// каждый такой показ удлиняет добор слова на одну карточку. Держим умеренным:
// при ~6 показах на слово это ≈1 аудио за слово — не растягивает, но и не редко.
export const AUDIO_EXTRA_CHANCE = 0.18;
...
export function pickExerciseType(
  ...
  if (needMc || needType) {
    // Аудио-экстра (П.1): слово ещё осваивается, но уже узнано хотя бы раз
    // (mc≥1 ⇒ был верный «выбор») — иногда вставляем аудио-карточку СВЕРХ
    // программы. Не двигает выученность (mode "audio"), гейт audioOk (звук есть
    // и не mute). audioOk слева ⇒ короткое замыкание: без аудио rnd не тратится,
    // существующая логика микса/relief остаётся побитово прежней.
    if (audioOk && mc >= 1 && rnd() < AUDIO_EXTRA_CHANCE) return "mc_audio_ru";

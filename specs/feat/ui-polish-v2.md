...
## Находки code-review (починены)

- **Хоткеи сквозь модал** (подтверждено): MC-упражнение слушает `keydown` на
  `window` — при открытом диалоге «1»/«A» отвечали бы на карточку ПОД модалом.
  Фикс: capture-глушитель printable-клавиш (`key.length === 1`) на время жизни
  диалога; Tab/Escape/Enter проходят. Регресс-тест в `Shell.ct.tsx` (красный
  без фикса — проверено отключением слушателя).
- `aria-label` стрика — честная плюрализация `pluralRu` («4 дня», «5 дней»)
  вместо «дн.» (как в остальном UI).
- Тёмная тема: тёплая подложка `rgba(26,26,23,.35)` на тёмном фоне почти
  неразличима — `[data-theme="dark"]` оверрайд `rgba(0,0,0,.55)`.
- Отклонено с обоснованием: Tab-trap без ветки shift (для двух кнопок цикл
  вперёд/назад идентичен — поведение корректно), `useEffect`-once (Shell
  размонтирует диалог на закрытии — каждое открытие свежий mount), общее
  состояние `speakSmart` между MC и теорией (by design — «тот же текст»),
  `doneToday` не гаснет ровно в полночь (см. ограничения).

...
- **Выход по логотипу из активной сессии — через подтверждение**: `goHome`
  ...
  сессию. Confirm только на пути логотипа: «К повторению» с экрана `Complete` и
  «Назад» из теории уходят без вопроса (там нечего терять).
...
из сессии (принял/отменил/вне сессии).

...
- `recordAnswer({ lessonKey, pt, quality: 0|1|2, mode: "mc"|"type", clientDay? })`
  ...
  клиента для стрика; отсутствует/невалиден → fallback на серверную UTC-дату
  (graceful для закэшированного старого фронта).
...
отдельной миграции данных не требуется.
...
- Backend [`convex/progress.test.ts`](../../convex/progress.test.ts): SM-2 с `mode`,
  ...
  невалидный или отсутствующий clientDay), идемпотентность `reclampSchedules`.

...
- **Confirm на логотипе:** клик по логотипу-«домой» во время активной сессии
  спрашивает «Выйти из тренировки?»; отмена оставляет сессию. На экране
  завершения (Complete) прерывать уже нечего — логотип уходит домой БЕЗ
  вопроса. Прочие пути («К повторению» с экрана завершения, «Назад» из
  теории) — тоже без confirm.
...
- **Confirm знает о завершении сессии.** `Complete` рендерится ВНУТРИ `Session`
  (`idx >= items.length`), `view.kind` в Shell остаётся `"session"` — без
  доп. сигнала логотип на финальном экране спрашивал бы про прерывание уже
  завершённой тренировки. Session получил опциональный `onComplete?`,
  вызываемый в `advance()` при `ni >= items.length` (момент перехода к
  Complete; не в рендере). Shell держит флаг `sessionDone`: ставится в
  `onComplete`, сбрасывается в `startReview`/`startLesson`; `goHome` спрашивает
  confirm только при `view.kind === "session" && !sessionDone`. Правка Session
  намеренно минимальна (один проп + один вызов) — параллельная ветка тоже
  правит Session.tsx.
...
- Новые тесты: юнит `pluralRu` (1/2/5/11/14/21/22/0); юнит на срез очереди по
  ...
  Complete («1 слово ждёт» / «2 слова ждут»); `Shell.ct.tsx` — 6 сценариев
  (теория не просмотрена → экран теории; просмотрена → сразу сессия; confirm
  логотипа принят → домой; отменён → сессия живёт; логотип на экране Complete
  — без confirm; вне сессии — без confirm); юнит useTheme на синк обоих
  theme-color-метатегов (явный dark на светлой ОС; перецикливание; живая
  смена ОС).
...
- `src/components/Shell.tsx` — confirm на goHome (с учётом `sessionDone`),
  useMemo для adaptSrs.
- `src/components/Session.tsx` — опциональный `onComplete?` (вызов в
  `advance()` при исчерпании очереди; минимальная правка).
...
- Вырожденный случай: если сессия стартует с ПУСТОЙ очередью, Session
  показывает Complete сразу, минуя `advance()` → `onComplete` не вызовется и
  логотип спросит confirm. Текущие билдеры очередей пустую очередь при
  доступной кнопке старта не отдают (fallback в `buildReviewQueue`), поэтому
  кейс недостижим из UI.
